...
Returning to our smaller dataset, we can see that 7% + 25% = 32% is pretty close to the 39% we actually observed. They aren't exactly the same simply because of sampling error, so that the observed correlation between SBP measures in this sample of n = 100 is `r round(with(data, cor(sbp_eos, sbp_bl)), 2)`, and since the actual correlation between the outcome and study arm is `r round(with(data, cor(sbp_eos, as.numeric(arm))), 2)` instead of zero. 
